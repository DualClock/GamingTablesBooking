...
-- ================================================
-- Скрипт создания базы данных "GamingTablesBooking"
-- для системы бронирования игровых столов
-- SQL Server 2019+ / LocalDB / Express
-- Collation: Cyrillic_General_100_CI_AS_KS_WS_SC
-- ================================================
...
BEGIN
    ...
    -- Индекс по роли для фильтрации
    CREATE INDEX IX_Users_Role ON dbo.Users(role);
END
GO

-- ================================================
-- Таблица Tables — игровые столы
-- ================================================
...
BEGIN
    ...
    -- Индекс по статусу для быстрого поиска свободных столов
...
BEGIN
    ...
    -- Индексы для быстрого поиска бронирований
    ...
    -- Композитный индекс для ��роверки доступности (конфликт времени)
    CREATE INDEX IX_Bookings_TableDateTime
        ON dbo.Bookings(table_id ASC, date ASC, time_start ASC, time_end ASC)
        INCLUDE (status);
END
GO
